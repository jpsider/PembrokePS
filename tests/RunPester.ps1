...
Set-Location C:\OPEN_PROJECTS\ProjectPembroke\PembrokePS
...
foreach ($Directory in $Directories)
{
    ...
    $files = Get-ChildItem .\PembrokePS\$Directory
    foreach ($file in $files)
    {
        # Source the file
        $FileName = $file.Name
        Write-Output "Importing file $FileName"
        . .\PembrokePS\$Directory\$FileName
    }
    # Execute Pester for the Directory.
    $results = Invoke-pester .\tests\$Directory\*.ps1 -CodeCoverage .\PembrokePS\$Directory\*.ps1 -PassThru
